...
const C = {
  bg: "#09090f", surface: "#111118", surface2: "#1a1a24", border: "#2a2a3a",
  text: "#e8e8f0", textSub: "#a0a0b8", muted: "#6a6a80",
  blue: "#4a7cff", green: "#2dd4a0", red: "#f06060",
};
const MONO = Platform.select({ ios: "Menlo", android: "monospace" });

const KEYS = { products: "@sp_products", units: "@sp_units", categories: "@sp_categories", history: "@sp_history", converters: "@sp_converters" };

const DEFAULT_PRODUCTS = [];
const DEFAULT_UNITS = [];
const DEFAULT_CATEGORIES = [];

async function dbSave(key, data) { try { await AsyncStorage.setItem(key, JSON.stringify(data)); } catch (e) { console.warn("DB Save:", e); } }
async function dbLoad(key, fb) { try { const r = await AsyncStorage.getItem(key); return r ? JSON.parse(r) : fb; } catch { return fb; } }
function freshCostRow(cats) { return { id: Date.now().toString() + Math.random(), categoryId: cats[0]?.id ?? "", amount: "" }; }
...
function ConvBadge({ symbol, value, color }) {
  ...
  return (
    <View style={{ backgroundColor: bg, borderWidth: 1, borderColor: bdr, borderRadius: 5, paddingHorizontal: 5, paddingVertical: 1, marginLeft: 3, marginTop: 1 }}>
      <Text style={{ fontSize: 9, fontFamily: MONO, fontWeight: "700", color }}>≈{symbol}{value.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</Text>
    </View>
  );
}
  ...
  return (
    <Modal visible transparent animationType="fade" onRequestClose={onClose}>
      <TouchableOpacity activeOpacity={1} onPress={onClose} style={{ flex: 1 }}>
        <View style={{ flex: 1 }}>
          <View style={{ position: "absolute", top, left: anchorLayout.px, width: anchorLayout.width, maxHeight: maxH, backgroundColor: C.surface2, borderRadius: 10, borderWidth: 1, borderColor: C.border, overflow: "hidden", elevation: 10, shadowColor: "#000", shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.5, shadowRadius: 16 }}>
            <ScrollView nestedScrollEnabled bounces={false} keyboardShouldPersistTaps="handled">
              {options.map((item, idx) => (
                <TouchableOpacity key={item.value} onPress={() => { onSelect(item.value); onClose(); }}
...
function KBInput({ scrollRef, scrollNodeRef, ...props }) {
...
export default function App() {
  ...
  // Track scroll position for keyboard scroll
  const scrollY = useRef(0);
  const [kbHeight, setKbHeight] = useState(0);

  // Keyboard listeners — track actual height
  useEffect(() => {
    const show = Keyboard.addListener(Platform.OS === "ios" ? "keyboardWillShow" : "keyboardDidShow", (e) => { setKbVisible(true); setKbHeight(e.endCoordinates.height); });
  ...
  useEffect(() => {
    (async () => {
      try {
        // One-time clear of old defaults
        ...
        const [p, u, c, h, cv] = await Promise.all([dbLoad(KEYS.products, []), dbLoad(KEYS.units, []), dbLoad(KEYS.categories, []), dbLoad(KEYS.history, []), dbLoad(KEYS.converters, [])]);
        setProducts(p); setUnits(u); setCategories(c); setHistory(h); setConverterPairs(cv);
        setSelectedProduct(p[0]?.id ?? ""); setSelectedUnit(u[0]?.id ?? ""); setResultUnitId(u[0]?.id ?? "");
        if (c.length > 0) setCostItems([freshCostRow(c)]);
      } catch (e) {
  ...
  function openDropdown(type, refKey, id) {
    ...
    if (ref) { ref.measureInWindow((px, py, width, height) => { setDropdown({ visible: true, type, id, anchor: { px, py, width, height } }); }); }
  }
  ...
  function getConversions(amountStr) {
    const amt = parseFloat(amountStr); if (isNaN(amt) || amt === 0) return [];
    return converterPairs.filter((cp) => cp.fromUnitId === selectedUnit && parseFloat(cp.rate) > 0)
      .map((cp) => { const u = units.find((u) => u.id === cp.toUnitId); return { symbol: u?.symbol ?? "", value: amt * parseFloat(cp.rate) }; }).filter((x) => x.symbol);
  }
  function convertedProfit(raw) { if (resultUnit?.type === "percentage") return (raw / (result?.totalCost || 1)) * 100; return raw * rate; }
  function fmt(raw) { const val = convertedProfit(raw); if (resultUnit?.type === "percentage") return `${Math.abs(val).toFixed(2)}%`; return `${resultUnit?.symbol}${Math.abs(val).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`; }

  function newCalculation() { setTitle(""); setSellingPrice(""); setCostItems([freshCostRow(categories)]); setResult(null); setConversionRate("1"); setSellingMode("price"); setTargetMargin(""); }
  ...
  function calculate() {
    const sp = parseFloat(sellingPrice); if (isNaN(sp)) return;
  ...
  function saveCalc() {
    ...
    setHistory((h) => [{ id: Date.now().toString(), title: title.trim() || product?.name || "Untitled", productName: product?.name ?? "", productId: selectedProduct, unitId: selectedUnit, unitSymbol: activeUnit?.symbol ?? "$", sellingPrice: parseFloat(sellingPrice), costItems: costItems.filter((ci) => parseFloat(ci.amount) > 0).map((ci) => ({ categoryName: categories.find((c) => c.id === ci.categoryId)?.name ?? ci.categoryId, amount: parseFloat(ci.amount) })), totalCost: result.totalCost, profitAmount: result.profit, profitPercent: result.percent, resultUnitSymbol: resultUnit?.symbol ?? activeUnit?.symbol ?? "$", conversionRate: rate, convertedProfit: convertedProfit(result.profit), date: new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" }) }, ...h]);
  ...
  const suggestedPrice = sellingMode === "margin" && targetMargin !== "" && totalCostLive > 0 ? totalCostLive * (1 + parseFloat(targetMargin) / 100) : null;
  function applyTargetMargin() {
    if (suggestedPrice === null) return;
    setSellingPrice(suggestedPrice.toFixed(2)); setSellingMode("price"); setTargetMargin("");
    const profit = suggestedPrice - totalCostLive;
    setResult({ profit, percent: totalCostLive > 0 ? (profit / totalCostLive) * 100 : 0, totalCost: totalCostLive });
    Keyboard.dismiss();
    setTimeout(() => { scrollViewRef.current?.scrollToEnd({ animated: true }); }, 400);
  }

  const toastTimer = useRef(null);
  function showToast(message, type) { if (toastTimer.current) clearTimeout(toastTimer.current); setToast({ message, type }); toastTimer.current = setTimeout(() => setToast(null), 2000); }
  const setRef = useCallback((key) => (el) => { if (el) dropdownRefs.current[key] = el; }, []);
  function renderConvBadges(amountStr, color) { const convs = getConversions(amountStr); if (convs.length === 0) return null; return (<View style={{ flexDirection: "row", flexWrap: "wrap", justifyContent: "flex-end", maxWidth: "50%", gap: 2 }}>{convs.map((cv, i) => <ConvBadge key={i} symbol={cv.symbol} value={cv.value} color={color} />)}</View>); }

  if (!dbReady) {
    return (<SafeAreaView style={{ flex: 1, backgroundColor: C.bg, alignItems: "center", justifyContent: "center" }}><ActivityIndicator size="large" color={C.blue} /><Text style={{ color: C.muted, fontSize: 12, fontFamily: MONO, marginTop: 12 }}>Loading data...</Text></SafeAreaView>);
  }
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }}>
      <View style={[s.header, { paddingTop: SAFE_TOP + 8 }]}>
        ...
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          {tab === "calc" && (<TouchableOpacity onPress={newCalculation} style={s.plusBtn}><Text style={{ color: "#fff", fontSize: 18, fontWeight: "700", marginTop: -1 }}>+</Text></TouchableOpacity>)}
      ...
      <View style={s.tabs}>
        {["calc", "history", "manage"].map((t) => (
          <TouchableOpacity key={t} onPress={() => setTab(t)} style={[s.tabBtn, tab === t && s.tabActive]}>
            <Text style={[s.tabText, tab === t && s.tabTextActive]}>{t === "calc" ? "Calculator" : t === "history" ? "History" : "Manage"}</Text>
          </TouchableOpacity>
        ))}
      </View>
        ...
        >
          <View style={s.card}>
            <View style={s.cardInner}>
              ...
              <KBInput scrollRef={scrollViewRef} placeholder="e.g. June Batch — iPhone 15" placeholderTextColor={C.border} value={title} onChangeText={setTitle} style={s.titleInput} />
            </View>
          </View>

          <View style={{ flexDirection: "row", gap: 8 }}>
            <View style={{ flex: 1 }} ref={setRef("product")} collapsable={false}>
              <DropdownButton label="Product" value={selectedProduct} options={products.map((p) => ({ value: p.id, label: p.name }))} onPress={() => openDropdown("product", "product")} onAdd={() => setSheet("product")} />
            </View>
            <View style={{ flex: 1 }} ref={setRef("inputCurrency")} collapsable={false}>
              <DropdownButton label="Input Currency" value={selectedUnit} options={units.map((u) => ({ value: u.id, label: `${u.symbol} ${u.label}` }))} onPress={() => openDropdown("inputCurrency", "inputCurrency")} onAdd={() => setSheet("unit")} />
            </View>
          </View>
          ...
          <View style={s.card}>
            <View style={s.sectionHeader}>
              <View style={{ flex: 1 }}><Text style={s.label}>Currency Converter</Text><Text style={s.hint}>Auto-converts cost rows to target currencies</Text></View>
              <TouchableOpacity onPress={addConverterPair} style={s.addPill}><Text style={s.addPillText}>+ Add</Text></TouchableOpacity>
            </View>
            {converterPairs.length === 0 ? (
              <View style={{ paddingHorizontal: 12, paddingBottom: 10 }}><Text style={{ fontSize: 9, color: C.border, fontFamily: MONO, fontStyle: "italic" }}>No converters yet. Tap + Add to set rates.</Text></View>
            ) : converterPairs.map((cp, idx) => {
              const fromU = units.find((u) => u.id === cp.fromUnitId); const toU = units.find((u) => u.id === cp.toUnitId);
              return (
                <View key={cp.id} style={[s.converterRow, idx > 0 && s.borderTop]}>
                  <Text style={s.converterOne}>1</Text>
                  <TouchableOpacity ref={setRef(`cf_${cp.id}`)} collapsable={false} onPress={() => openDropdown("converterFrom", `cf_${cp.id}`, cp.id)} style={s.miniSelect}>
                    <Text style={s.miniSelectText} numberOfLines={1}>{fromU?.symbol} {fromU?.label}</Text><Text style={s.chevron}>▼</Text>
                  </TouchableOpacity>
                  <Text style={s.converterEq}>=</Text>
                  <KBInput scrollRef={scrollViewRef} keyboardType="decimal-pad" placeholder="0.00" placeholderTextColor={C.border} value={cp.rate} onChangeText={(v) => updateConverterPair(cp.id, "rate", v)} style={s.converterInput} />
                  <TouchableOpacity ref={setRef(`ct_${cp.id}`)} collapsable={false} onPress={() => openDropdown("converterTo", `ct_${cp.id}`, cp.id)} style={[s.miniSelect, { flex: 1 }]}>
                    <Text style={s.miniSelectText} numberOfLines={1}>{toU?.symbol} {toU?.label}</Text><Text style={s.chevron}>▼</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={() => removeConverterPair(cp.id)} style={s.removeBtn}><Text style={{ color: C.muted, fontSize: 11 }}>✕</Text></TouchableOpacity>
                </View>
              );
            })}
          </View>
          ...
          <View style={s.card}>
            <View style={s.sectionHeader}>
              ...
              <TouchableOpacity onPress={() => setSheet("category")}><Text style={{ fontSize: 10, color: C.blue, fontWeight: "600", fontFamily: MONO }}>+ Category</Text></TouchableOpacity>
            </View>
              ...
              return (
                <View key={ci.id} style={[s.costRow, idx > 0 && s.borderTop]}>
                  <View style={s.costCatRow}>
                    <TouchableOpacity ref={setRef(`cc_${ci.id}`)} collapsable={false} onPress={() => openDropdown("costCategory", `cc_${ci.id}`, ci.id)} style={{ flexDirection: "row", alignItems: "center", flex: 1 }}>
                      <Text style={{ color: C.text, fontSize: 13, fontWeight: "600", flex: 1 }}>{catName}</Text><Text style={[s.chevron, { marginRight: 6 }]}>▼</Text>
                    </TouchableOpacity>
                    <TouchableOpacity onPress={() => removeCostRow(ci.id)} style={s.removeBtn}><Text style={{ color: C.muted, fontSize: 11 }}>✕</Text></TouchableOpacity>
                  </View>
                  <View style={s.inputRow}>
                    <Text style={s.currencySymbol}>{activeUnit?.symbol}</Text>
                    <KBInput scrollRef={scrollViewRef} keyboardType="decimal-pad" placeholder="0.00" placeholderTextColor={C.border} value={ci.amount} onChangeText={(v) => updateCostRow(ci.id, "amount", v)} style={s.amountInput} />
                    {convBadges ? convBadges : (<Text style={{ fontSize: 9, color: C.muted, fontFamily: MONO }}>{activeUnit?.label}</Text>)}
                  </View>
                </View>
              );
            })}
            <View style={s.costFooter}>
              <TouchableOpacity onPress={addCostRow} style={{ flexDirection: "row", alignItems: "center" }}>
                <Text style={{ color: C.blue, fontSize: 14, marginRight: 3 }}>+</Text><Text style={{ color: C.blue, fontSize: 11, fontWeight: "600" }}>Add Cost Row</Text>
              </TouchableOpacity>
              <View style={{ flexDirection: "row", alignItems: "center", flexWrap: "wrap", justifyContent: "flex-end" }}>
                ...
                <Text style={{ fontSize: 11, fontFamily: MONO, color: C.red, fontWeight: "700" }}>{activeUnit?.symbol}{totalCostLive.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</Text>
          ...
          <View style={s.card}>
            <View style={{ paddingHorizontal: 12, paddingTop: 10, paddingBottom: 6, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <View style={{ flexDirection: "row", borderRadius: 8, overflow: "hidden", borderWidth: 1, borderColor: C.border }}>
                <TouchableOpacity onPress={() => { setSellingMode("price"); setTargetMargin(""); }} style={{ paddingHorizontal: 10, paddingVertical: 4, backgroundColor: sellingMode === "price" ? C.blue : "transparent" }}>
                ...
                <TouchableOpacity onPress={() => { setSellingMode("margin"); setResult(null); }} style={{ paddingHorizontal: 10, paddingVertical: 4, backgroundColor: sellingMode === "margin" ? C.green : "transparent" }}>
            ...
            {sellingMode === "price" ? (
              <View style={{ paddingHorizontal: 14, paddingBottom: 12 }}>
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  ...
                  <KBInput scrollRef={scrollViewRef} keyboardType="decimal-pad" placeholder="0.00" placeholderTextColor={C.border} value={sellingPrice} onChangeText={(v) => { setSellingPrice(v); setResult(null); }} style={{ flex: 1, color: C.text, fontFamily: MONO, fontWeight: "600", fontSize: 20, padding: 0, marginLeft: 6 }} />
              ...
              <View style={{ paddingHorizontal: 14, paddingBottom: 14 }}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: C.surface2, borderRadius: 10, borderWidth: 1, borderColor: "rgba(45,212,160,0.35)", paddingHorizontal: 10, paddingVertical: 8, marginBottom: 10 }}>
                  ...
                  <KBInput scrollRef={scrollViewRef} keyboardType="decimal-pad" placeholder="e.g. 25" placeholderTextColor={C.border} value={targetMargin} onChangeText={(v) => { setTargetMargin(v); setResult(null); }} style={{ flex: 1, color: C.text, fontFamily: MONO, fontWeight: "700", fontSize: 18, padding: 0, textAlign: "right" }} />
                ...
                {suggestedPrice !== null && totalCostLive > 0 ? (
                  <View style={{ borderRadius: 10, borderWidth: 1, borderColor: "rgba(45,212,160,0.3)", backgroundColor: "rgba(45,212,160,0.04)", paddingHorizontal: 14, paddingVertical: 12 }}>
                    ...
                    <View style={{ flexDirection: "row", alignItems: "baseline", gap: 4, marginBottom: 8 }}>
                      ...
                      <Text style={{ color: C.green, fontWeight: "700", fontSize: 28 }}>{suggestedPrice.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</Text>
                    </View>
                    <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                      ...
                      <Text style={{ fontSize: 9, fontFamily: MONO, color: C.green, fontWeight: "600" }}>+{activeUnit?.symbol}{(suggestedPrice - totalCostLive).toFixed(2)} profit</Text>
                    </View>
                    {renderConvBadges(suggestedPrice.toFixed(2), C.green) && (<View style={{ marginBottom: 10 }}>{renderConvBadges(suggestedPrice.toFixed(2), C.green)}</View>)}
                  ...
                  <Text style={{ fontSize: 9, fontFamily: MONO, color: C.muted, fontStyle: "italic", textAlign: "center", paddingVertical: 4 }}>Add cost items first to calculate suggested price.</Text>
                ) : null}
              </View>
            )}
          </View>
              ...
              <View style={{ padding: 16 }}>
                ...
                <View style={{ flexDirection: "row", gap: 6, marginBottom: 14 }}>
                  {["amount", "percent", "both"].map((v) => (
                    <TouchableOpacity key={v} onPress={() => setProfitView(v)} style={[s.toggleBtn, profitView === v && s.toggleActive]}>
                      <Text style={[s.toggleText, profitView === v && s.toggleTextActive]}>{v === "amount" ? "Amount" : v === "percent" ? "%" : "Both"}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
                <Text style={[s.label, { marginBottom: 6 }]}>Result Currency</Text>
                <TouchableOpacity ref={setRef("resultCurrency")} collapsable={false} onPress={() => openDropdown("resultCurrency", "resultCurrency")} style={[s.inputRow, { marginBottom: 14 }]}>
                  <Text style={{ color: C.text, fontSize: 13, fontWeight: "600", flex: 1 }}>{resultUnit?.symbol} — {resultUnit?.label}</Text><Text style={s.chevron}>▼</Text>
                </TouchableOpacity>
                {!isSameUnit && resultUnit?.type !== "percentage" && activeUnit?.type !== "percentage" && (<>
                  ...
                  <View style={[s.inputRow, { borderColor: "rgba(74,124,255,0.4)", marginBottom: 14 }]}>
                    ...
                    <KBInput scrollRef={scrollViewRef} keyboardType="decimal-pad" placeholder="1.00" placeholderTextColor={C.border} value={conversionRate} onChangeText={setConversionRate} style={{ flex: 1, color: C.text, fontFamily: MONO, fontWeight: "600", fontSize: 14, padding: 0, marginLeft: 6 }} />
                    <Text style={{ color: C.blue, fontFamily: MONO, fontWeight: "700", fontSize: 12 }}>{resultUnit?.symbol}</Text>
                  </View>
                </>)}
                <View style={s.divider} />
                <View style={{ flexDirection: "row", alignItems: "flex-end", marginBottom: 16 }}>
                  {(profitView === "amount" || profitView === "both") && (<View><Text style={s.resultSubLabel}>{!isSameUnit ? `Amount (${resultUnit?.symbol})` : "Amount"}</Text><Text style={{ fontSize: 28, fontWeight: "700", color: accentColor }}>{fmt(result.profit)}</Text></View>)}
                  {(profitView === "percent" || profitView === "both") && (<View style={profitView === "both" ? { marginLeft: "auto", alignItems: "flex-end" } : {}}><Text style={s.resultSubLabel}>Margin</Text><Text style={{ fontSize: 20, fontFamily: MONO, fontWeight: "700", color: accentColor }}>{result.percent >= 0 ? "+" : ""}{result.percent.toFixed(1)}%</Text></View>)}
                </View>
                <View style={s.summaryBox}>
                  <View style={s.summaryRow}><Text style={s.summaryLabel}>Selling Price</Text><Text style={s.summaryVal}>{activeUnit?.symbol}{parseFloat(sellingPrice).toFixed(2)}</Text></View>
                  {costItems.filter((ci) => parseFloat(ci.amount) > 0).map((ci) => (<View key={ci.id} style={s.summaryRow}><Text style={[s.summaryLabel, { color: C.muted }]}>{categories.find((c) => c.id === ci.categoryId)?.name}</Text><Text style={s.summaryVal}>−{activeUnit?.symbol}{parseFloat(ci.amount).toFixed(2)}</Text></View>))}
                  <View style={[s.summaryRow, s.borderTop, { paddingTop: 6 }]}><Text style={s.summaryLabel}>Total Cost</Text><Text style={[s.summaryVal, { color: C.red }]}>−{activeUnit?.symbol}{result.totalCost.toFixed(2)}</Text></View>
                  <View style={[s.summaryRow, s.borderTop, { paddingTop: 6 }]}><Text style={[s.summaryLabel, { fontWeight: "700", color: accentColor }]}>{isLoss ? "Net Loss" : "Net Profit"}{!isSameUnit ? ` (${resultUnit?.symbol})` : ""}</Text><Text style={[s.summaryVal, { fontWeight: "700", color: accentColor }]}>{fmt(result.profit)}</Text></View>
                  {!isSameUnit && resultUnit?.type !== "percentage" && (<Text style={{ fontSize: 8, fontFamily: MONO, color: C.muted, textAlign: "right", marginTop: 4 }}>Rate: 1{activeUnit?.symbol} = {rate}{resultUnit?.symbol}</Text>)}
                </View>
                <View style={s.progressTrack}><View style={[s.progressFill, { width: `${Math.min(Math.abs(result.percent), 100)}%`, backgroundColor: accentColor }]} /></View>
                <TouchableOpacity onPress={saveCalc} style={s.saveBtn}><Text style={{ color: C.textSub, fontSize: 13, fontWeight: "500" }}>Save to History</Text></TouchableOpacity>
              </View>
            </View>
          )}
        </ScrollView>
      ...
      {tab === "history" && (
        <ScrollView style={{ flex: 1 }} contentContainerStyle={s.scrollContent}>
          ...
          {history.length === 0 ? (
            <View style={{ alignItems: "center", paddingVertical: 80 }}><Text style={{ fontSize: 40, marginBottom: 10 }}>📜</Text><Text style={{ color: C.muted, fontSize: 13 }}>No saved calculations yet</Text></View>
          ) : (<View style={{ gap: 8 }}>
            {history.map((h) => {
              ...
              return (<View key={h.id} style={s.card}>
                <View style={{ padding: 12, paddingBottom: 8 }}><View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 4 }}><View style={{ flex: 1, marginRight: 6 }}><Text style={{ fontWeight: "600", fontSize: 13, color: C.text }} numberOfLines={1}>{h.title}</Text><Text style={{ fontSize: 9, fontFamily: MONO, color: C.muted, marginTop: 2 }}>{h.productName} · {h.date}</Text></View><View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}><View style={{ backgroundColor: loss ? "rgba(240,96,96,0.1)" : "rgba(45,212,160,0.1)", borderRadius: 999, paddingHorizontal: 6, paddingVertical: 2 }}><Text style={{ fontSize: 9, fontFamily: MONO, fontWeight: "700", color: ac }}>{loss ? "LOSS" : "PROFIT"}</Text></View><TouchableOpacity onPress={() => deleteHistory(h.id)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}><Text style={{ color: C.muted, fontSize: 11 }}>🗑</Text></TouchableOpacity></View></View></View>
                <View style={{ paddingHorizontal: 12, paddingBottom: 8, flexDirection: "row" }}>
                  {[{ l: "Sell", v: `${h.unitSymbol}${h.sellingPrice.toFixed(2)}` }, { l: "Cost", v: `${h.unitSymbol}${h.totalCost.toFixed(2)}` }, { l: "Profit", v: dp, c: ac, b: true }, { l: "Margin", v: `${h.profitPercent >= 0 ? "+" : ""}${h.profitPercent.toFixed(1)}%`, c: ac, b: true, r: true }].map((x, i) => (
                    <View key={i} style={{ flex: 1, alignItems: x.r ? "flex-end" : "flex-start" }}><Text style={{ fontSize: 8, fontFamily: MONO, color: C.muted, textTransform: "uppercase", marginBottom: 2 }}>{x.l}</Text><Text style={{ fontFamily: MONO, fontSize: 11, fontWeight: x.b ? "700" : "400", color: x.c || C.text }}>{x.v}</Text></View>
                  ))}
                </View>
                {h.costItems.length > 0 && (<View style={{ paddingHorizontal: 12, paddingBottom: 8, flexDirection: "row", flexWrap: "wrap", gap: 4 }}>{h.costItems.map((ci, i) => (<View key={i} style={{ backgroundColor: C.surface2, borderWidth: 1, borderColor: C.border, borderRadius: 5, paddingHorizontal: 6, paddingVertical: 1 }}><Text style={{ fontSize: 8, fontFamily: MONO, color: C.muted }}>{ci.categoryName}: {h.unitSymbol}{ci.amount.toFixed(2)}</Text></View>))}</View>)}
                <TouchableOpacity onPress={() => loadFromHistory(h)} style={s.loadBtn}><Text style={{ fontSize: 9, fontFamily: MONO, fontWeight: "600", color: C.blue, letterSpacing: 1.5, textTransform: "uppercase" }}>Load & Edit →</Text></TouchableOpacity>
              </View>);
            })}
          </View>)}
        </ScrollView>
      ...
      {/* ─── FIXED MODAL WITH KEYBOARD HANDLING ─── */}
      <Modal 
        visible={sheet !== null} 
        transparent 
        animationType="slide" 
        ...
        >
          <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.7)", justifyContent: "flex-end" }}>
            <TouchableOpacity 
              style={{ flex: 1 }} 
              activeOpacity={1} 
              onPress={() => { Keyboard.dismiss(); setSheet(null); }} 
            />
              ...
              {sheet === "product" && (
                <>
                  ...
                  <TouchableOpacity 
                    ...
                    style={[s.sheetSaveBtn, !newProductName.trim() && { opacity: 0.4 }]}
                  >
              ...
              {sheet === "unit" && (
                <>
                  ...
                  <View style={{ flexDirection: "row", gap: 10 }}>
                    ...
                    <View style={{ width: 72 }}>
                      ...
                      <TextInput 
                        ...
                        returnKeyType="done" 
                        style={[s.sheetInput, { textAlign: "center", fontFamily: MONO }]} 
                      />
                    </View>
                  </View>
                  ...
                  <View style={{ flexDirection: "row", gap: 6, marginBottom: 14 }}>
                    {["currency", "percentage"].map((t) => (
                      <TouchableOpacity 
                        ...
                        style={[s.toggleBtn, { paddingVertical: 8 }, newUnitType === t && s.toggleActive]}
                      >
                        <Text style={[s.toggleText, { fontSize: 13 }, newUnitType === t && s.toggleTextActive]}>
                  ...
                  <TouchableOpacity 
                    ...
                    style={[s.sheetSaveBtn, (!newUnitLabel.trim() || !newUnitSymbol.trim()) && { opacity: 0.4 }]}
                  >
                    <Text style={s.sheetSaveBtnText}>Save</Text>
                  </TouchableOpacity>
                </>
              ...
              {sheet === "category" && (
                <>
                  ...
                  <TouchableOpacity 
                    ...
                    style={[s.sheetSaveBtn, !newCatName.trim() && { opacity: 0.4 }]}
                  >
                    <Text style={s.sheetSaveBtnText}>Save</Text>
                  </TouchableOpacity>
                </>
      ...
      {toast && (<View style={{ position: "absolute", bottom: 40, left: 20, right: 20, backgroundColor: toast.type === "green" ? C.green : C.red, borderRadius: 12, paddingVertical: 12, paddingHorizontal: 16, flexDirection: "row", alignItems: "center", gap: 8, elevation: 8, shadowColor: "#000", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8 }}><Text style={{ fontSize: 16 }}>{toast.type === "green" ? "✓" : "🗑"}</Text><Text style={{ color: "#fff", fontSize: 13, fontWeight: "600", flex: 1 }}>{toast.message}</Text></View>)}
    </SafeAreaView>
  );
}
...
const s = StyleSheet.create({
  ...
  sheetContainer: { backgroundColor: C.surface, borderTopLeftRadius: 20, borderTopRightRadius: 20, borderTopWidth: 1, borderTopColor: C.border, paddingHorizontal: 20, paddingTop: 12, paddingBottom: Platform.OS === "ios" ? 40 : 24 },
